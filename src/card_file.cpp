...
int getCardCount(const char *pPath) {
    FILE *pFile = NULL;
	Card *pCard = (Card *)malloc(sizeof(Card)); // 手动分配内存
    int nCount = 0;
    ...
    // 逐条读取记录
    while (fread(pCard, sizeof(Card), 1, pFile) == 1) {
        nCount++;
    }

    // 检查是否因错误终止（非文件结尾）
    if (!feof(pFile)) {
        fprintf(stderr, "警告：文件读取异常，统计结果可能不准确\n");
    }
...
int updateCard(const Card *pCard, const char *pPath, int nIndex)
{
	...
	iofile.seekp((__int64_t)lPosition - sizeof(Card), ios_base::beg);
	// 更新消费信息到文件
